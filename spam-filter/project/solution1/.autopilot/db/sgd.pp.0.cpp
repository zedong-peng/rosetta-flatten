...
# 44 "sgd.cpp" 2
  ...
  typedef ap_uint<512> VectorFeatureType;
  const unsigned int F_VECTOR_SIZE = 512 / 32;



  typedef ap_fixed<16,4> DataType;
  typedef ap_uint<512> VectorDataType;
  const unsigned int D_VECTOR_SIZE = 512 / 16;



  typedef ap_uint<8> LabelType;
  typedef ap_uint<32> VectorLabelType;
  const unsigned int L_VECTOR_SIZE = 32 / 8;
  ...
  typedef ap_uint<12> IdxFixed;
  typedef ap_fixed<12, 4> LutInFixed;
  typedef ap_fixed<12, 2> LutOutFixed;
# 94 "sgd.cpp"
# 1 "./sgd.h" 1
...
__attribute__((sdx_kernel("SgdLR", 0))) void SgdLR( VectorDataType data[NUM_FEATURES * NUM_TRAINING / D_VECTOR_SIZE],
 ...
 READ_TRAINING_DATA: for (int i = 0; i < NUM_FEATURES / D_VECTOR_SIZE; i ++ )
  {
    ...
    READ_TRAINING_DATA_INNER: for (int j = 0; j < D_VECTOR_SIZE; j ++ )
      training_instance[i * D_VECTOR_SIZE + j].range(16 -1, 0) = tmp_data.range((j+1)*16 -1, j*16);
  }
}
...
#pragma HLS INLINE
 const int unroll_factor = 32;
  ...
  DOT: for (int i = 0; i < NUM_FEATURES / 32; i++)
  {

    DOT_INNER: for(int j = 0; j < 32; j++)
    {
      FeatureType term = param[i*32 +j] * feature[i*32 +j];
      result += term;
    }
  }
  return result;
...
#pragma HLS INLINE
 IdxFixed index;
  ...
  {
    in = -in;
    index = (IdxFixed)LUT_SIZE - (IdxFixed)(((TmpFixed)in) << (12 - 4));
  }
  else
    index = ((TmpFixed)in) << (12 - 4);
  return lut[index];
}
...
#pragma HLS INLINE
 const int unroll_factor = 32;
  ...
  GRAD: for (int i = 0; i < NUM_FEATURES / 32; i++)
  {

    GRAD_INNER: for (int j = 0; j < 32; j++)
...
#pragma HLS INLINE
 const int unroll_factor = 32;
  ...
  UPDATE: for (int i = 0; i < NUM_FEATURES / 32; i++)
  {

    UPDATE_INNER: for (int j = 0; j < 32; j++)
 ...
 STREAM_OUT: for (int i = 0; i < NUM_FEATURES / F_VECTOR_SIZE; i ++ )
  {
    ...
    STREAM_OUT_INNER: for (int j = 0; j < F_VECTOR_SIZE; j ++ )
      tmp_theta.range((j+1)*32 -1, j*32) = theta_local[i * F_VECTOR_SIZE + j].range(32 -1, 0);
    theta[i] = tmp_theta;
  }
...
__attribute__((sdx_kernel("SgdLR", 0))) void SgdLR( VectorDataType data[NUM_FEATURES * NUM_TRAINING / D_VECTOR_SIZE],
            ...
            bool writeOutput)
{
#line 8 "/data/zedongpeng/HLSBatchProcessor/data/kernels/rosetta_flatten/spam-filter/run_hls.tcl"
  ...
  if (readLabels)
  {
    ...
    LABEL_CP: for (int i = 0; i < NUM_TRAINING / L_VECTOR_SIZE; i ++ )
    {
      ...
      LABEL_CP_INNER: for (int j = 0; j < L_VECTOR_SIZE; j ++ )
        label_local[i * L_VECTOR_SIZE + j].range(8 -1, 0) = tmp_label.range((j+1)*8 -1, j*8);
    }
  }
